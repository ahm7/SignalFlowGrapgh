...
public enum Arrow {
	...
	public static void draw(final Graphics2D gfx, final Point2D start,
			final Point2D end, final Stroke lineStroke,
		...
		if (deltax == 0.0d) {
			...
			result = Math.atan((starty - end.getY()) / deltax)
					+ (startx < end.getX() ? Math.PI : 0);
		}
		...
		// gfx.drawLine((int) startx, (int) starty, (int) (end.getX() + cx),
		// (int) (end.getY() + cy));
	}
